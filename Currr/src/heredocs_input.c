...
/*   Updated: 2025/05/02 17:32:48 by vagarcia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
						...
						bool expand_vars)
{
	...
	while (42)
	{
		...
		line = readline("> ");
		// If readline returns NULL, either EOF or signal interrupted
		if (!line)
		{
			// If it was a signal, don't show EOF warning
			if (g_signal_received)
				return;
...
void	handle_heredoc(t_cmd *cmd, char *eof)
{
	...
	// Use the new direct approach instead of forking
	exit_code = handle_heredoc_direct(cmd, clean_eof, temp_name, expand_vars);
	
	// Reset signals and only create redirection node if successful
	setup_signals();
	if (exit_code == 0)
		create_redir_node(cmd, REDIR_HEREDOC, temp_name);
	else if (exit_code == 130)
		cmd->shell->signal_status = 1; // Set signal status for the shell

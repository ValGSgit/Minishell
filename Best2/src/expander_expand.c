...
/**
 * Handles special cases for $ when it's followed by special characters
 * Returns a string if the case is handled, NULL otherwise
 */
...
/**
 * Handles special characters after a $ sign
 * Returns a string if handled, NULL otherwise
 */
static char *handle_quoted_content(char *str, int start, int *i)
{
    char quote_type = str[start];
    int quote_start = start + 1;
    int end = quote_start;
...
/**
 * Extracts the variable name after a $ sign
...
/**
 * Expands a variable in a string

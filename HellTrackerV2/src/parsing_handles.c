		...
		t_shell *shell)
{
	...
	/* Handle edge case of redirection followed by pipe */
	if (tokens[*i + 1] && is_redirection(tokens[*i + 1]) && 
		(ft_strcmp(tokens[*i + 1], ">") == 0 || 
		 ft_strcmp(tokens[*i + 1], ">>") == 0))
	{
		/* If there are no args yet, create an empty arg to ensure proper parsing */
		if (!(*current)->args)
		{
			(*current)->args = ft_calloc(1, sizeof(char *));
			if (!(*current)->args)
				return;
		}

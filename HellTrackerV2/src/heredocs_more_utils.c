	...
	bool expand_vars)
{
	...
	while (1)
	{
		...
		{
			xfree(line);  /* Use xfree instead of free */
			break;
		}
		if (expand_vars && ft_strchr(line, '$'))
		{
			...
			xfree(processed);  /* Use xfree instead of free */
		}
		else
...
char	*handle_expanded_line(char *arg, char *expanded_line)
{
	...
	xfree(arg);  /* Use xfree instead of free */
	return (new_line);
}

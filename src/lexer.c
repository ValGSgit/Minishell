...
static int	process_special_char(char **tokens, int *count, char **input, t_shell *shell)
{
	if (is_special_char(**input))
	{
		if (**input == '>' && *(*input + 1) == '>') 
		{
			if (add_token(tokens, count, *input, *input + 2, shell) < 0)
			{
				return (-1);
			}
			(*input) += 2;
		...
		else if (add_token(tokens, count, *input, *input + 1, shell) < 0)
			return (-1);
		(*input)++;
		return (1);
	}
...
char	**lexer(char *input, t_shell *shell)
{
	...
	{
		//printf("Processing: '%c'\n", *input);
		//printf("Current token start: %s\n", lx.start);
		if (handle_quotes(&input, &lx.in_quote, &lx.quote_char))
			continue ;
		if (!lx.in_quote && !is_redir_without_space(&input) && (ft_isspace(*input) || is_special_char(*input)))
		{
			if (lx.start != input && add_token(tokens, &lx.tokcount, lx.start,
					input, shell) < 0)
				return (free_tokens(tokens), NULL);
		...
		else if (!lx.in_quote && is_redir_without_space(&input))
		{
			if (lx.start != input && add_token(tokens, &lx.tokcount, lx.start, input, shell) < 0)
				return (free_tokens(tokens), NULL);
			if (process_special_char(tokens, &lx.tokcount, &input, shell) < 0)
				return (free_tokens(tokens), NULL);
			lx.start = input;

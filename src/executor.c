...
/*   Updated: 2025/03/31 12:30:40 by vagarcia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void execute_single_command(t_cmd *cmd, t_shell *shell)
{
    ...
    if (access(cmd->args[i], X_OK) != 0)
    {
        if (check_file_permissions(cmd->args[i]) == -1)
        {
            // write(2, "minishell: ", 11);
        ...
        else if (check_file_permissions(cmd->args[i]) == -2)
        {
    ...
    if (pid == 0)
    {
        ...
        execve(cmd->args[i], cmd->args, shell->env);
        perror(cmd->args[i]); // Print error if execve fails
    ...
    while (cmd)
    {
        ...
        else // Parent process
        {
            ...
            if (cmd->next)
                ...
                prev_pipe_in = 0; // Update for the next iteration
            cmd = cmd->next;
        }
    }
    while (wait(NULL) > 0);

...
/*   Updated: 2025/04/13 15:03:43 by vagarcia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static const char *get_random_temp_name(void)
{
	...
	start_pos = ft_strlen("/tmp/heredoc_"); // Position of the first 'X'
	...
	i = -1;
	while (++i < 6)
		temp_name[start_pos + i] = '0' + (random_bytes[i] % 10); // Generate digits
	temp_name[start_pos + 6] = '\0';
	return (temp_name);
}
...
static char *clean_empty_expansions(char *arg)
{
	...
	while (arg[i])
	{
		...
		{
			//int var_start = i;
			int var_len = 1; // Start with $ character
			
			// Check if this is a proper variable reference (letter or underscore followed by letters, digits, or underscores)
			if (ft_isalpha(arg[i + var_len]) || arg[i + var_len] == '_')
			{
				var_len++;
				while (ft_isalnum(arg[i + var_len]) || arg[i + var_len] == '_')
					var_len++;
					
				// If this variable doesn't exist (it was completely removed during expansion),
				// there will be a $ character directly before where the value should be
				if (var_len > 1)
				{
					i += var_len; // Skip the entire variable reference
					continue;
				}
			}
		}
		
		// Handle other cases like $SPACE, $EOF, $$, $/

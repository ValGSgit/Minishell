...
/*   Updated: 2025/04/29 21:34:36 by vagarcia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	is_invalid_number(const char *str)
{
	...
	while (str[i] && is_whitespace(str[i]))
		i++;
	/* Valid if we have at least one digit and reached the end of the string */
	if (!str[i] && has_digits)
		return (0);
	return (1);
}
		...
		long long result)
{
	...
	/* Special handling for LLONG_MIN edge case */
	...
	/* Process current digit and recurse for next */
...
static long long	ft_atol_improved(const char *str)
{
	...
	/* Skip any zeros that appear after the sign */
	while (str[i] && str[i] == '0')
		i++;
